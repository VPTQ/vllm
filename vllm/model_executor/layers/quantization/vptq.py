...
def optimized_dequantize_gemm(
    ...
    invert_perm = torch.argsort(perm.view(torch.uint16).to(torch.int64)).to(
        torch.uint16).view(torch.int16)
...
class VPTQLinearMethod(LinearMethodBase):
    ...
    @staticmethod
    def quantized_weight_loader(indice_sizes):  # specific for layer.indices
        ...
        def wrap_weight_loader(param: torch.nn.Parameter,
            ...
            if loaded_shard_id is not None:
                param_data = param_data.narrow(
                    1, sum(shard_sizes[:_loaded_shard_id]),
                    shard_sizes[_loaded_shard_id])

            # split for TP
            loaded_weight = loaded_weight.narrow(1, offset, end - offset)
    ...
    def apply(
        ...
        return generic_dequantize_gemm(x, indices, centroids, res_centroids,
